...
{finalangle= analogRead(A0)*0.267;}

int changeSangle(int i , int angle_0)
{int angle = i*angle
    int ret = 0;
    int req_adc = angle/0.267 + refangle;
    int current_adc = analogRead(A0);
    Serial.print("current angle:");
    Serial.println((current_adc - refangle)*0.267);
    if(current_adc - req_adc>10)
    { 
       case_1(current_adc,req_adc);
    }
    else if(current_adc - req_adc<-10)
    {
